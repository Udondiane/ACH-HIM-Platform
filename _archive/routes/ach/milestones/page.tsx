...
export default async function MilestonesPage() {
  ...
  const retentionPaid = retentionMs.filter(r => r.state === 'paid').reduce((s, r) => s + Number(r.amount), 0);
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <PageHeader
        ...
        description="Placement and retention milestone payments across all partners. Retention milestone payments ringfence into the candidate Development Fund."
      />
      ...
      {rows.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Milestones appear when placements have payment schedules attached."
          />
        </Card>
      ) : (
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className="border-b-[0.5px] border-ach-border last:border-0 hover:bg-ach-page/40">
                  ...
                  <Td className="text-ach-navy/80">{r.placements?.partners?.name ?? '—'}</Td>
