...
import { ArrowRight, CheckCircle2, Circle, Loader2, Lock, AlertTriangle } from 'lucide-react';
...
export default async function CandidateAssessChooserPage({ params }: { params: { id: string } }) {
  ...
  // Find the cohort & project the candidate is linked to.
  const { data: cc } = await supabase
    .from('cohort_candidates')
    .select('cohort_id, cohorts(id, cohort_ref, name, project_id, projects(id, project_ref, name, is_locked))')
    .eq('candidate_id', params.id)
    .maybeSingle();
  const cohort = (cc as any)?.cohorts ?? null;
