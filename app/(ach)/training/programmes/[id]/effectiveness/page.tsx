...
export default async function TrainingProgrammeEffectivenessPage({ params }: { params: { id: string } }) {
  const supabase = createClient();

  const [prog, learningOutcomes, outcomeMap, enrolments, factors] = await Promise.all([
    ...
    safeFetch<any[]>(() => supabase.from('training_enrolments').select('id, candidate_id, status, enrolled_date, completed_date').eq('programme_id', params.id), []),
  ...
  const enrolled = (enrolments as any[]).map(e => e.candidate_id);
  const completed = (enrolments as any[]).filter(e => e.status === 'completed').map(e => e.candidate_id);
  const inProgress = (enrolments as any[]).filter(e => e.status === 'enrolled').length;
  const withdrawn = (enrolments as any[]).filter(e => e.status === 'withdrawn').length;
